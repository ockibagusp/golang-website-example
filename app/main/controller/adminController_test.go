...
func TestDeletePermanentlyByID(t *testing.T) {
	...
	// delete user subali (id=3)
	err := newUserService(db).Delete(business.InternalContext{}, 3)
	if err != nil {
	...
	testCases := []struct {
		...
		/*
			delete permanently by id [admin]
		*/
		{
			name:   "delete permanently by id [admin] to GET it failure: id=1",
			expect: ADMIN,
			path:   "1",
			// HTTP response status: 403 Forbidden
			status: http.StatusForbidden,
		...
		{
			name:   "delete permanently by id [admin] to GET it success: id=2",
			expect: ADMIN,
			path:   "3",
			// HTTP response status: 200 OK
			status: http.StatusOK,
			...
			htmlFlashSuccess: regex{
				mustCompile: `<strong>success:</strong> (.*)`,
				actual:      `<strong>success:</strong> success permanently user: subali!`,
			},
		},
		{
			name:   "delete permanently by id [admin] to GET it failure: id=-1",
		...
		/*
			delete permanently by id [sugriwa]
		*/
		{
			name:   "delete permanently by id [sugriwa] to GET it success: id=1",
			expect: SUGRIWA,
			path:   "1",
			// HTTP response status: 404 Not Found
			status: http.StatusNotFound,
		...
		*/
		{
			name:   "delete permanently by id [no-auth] to GET it failure: id=1",
			expect: ANONYMOUS,
			// HTTP response status: 404 Not Found
			status: http.StatusNotFound,
			jsonMessageError: regex{
	...
	for _, test := range testCases {
		...
		*/
		...
		t.Run(test.name, func(t *testing.T) {
			// @route: exemple "/admin/delete/permanently/3"
			result = noAuth.GET("/admin/delete/permanently/{id}", test.path).
				Expect().
				Status(test.status)
